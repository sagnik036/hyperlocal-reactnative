...
export default function JobDetails({ route }) {
  ...
  const jobD = () => {
    axios.get(`${API_URl}/jobs-details/${route.params.paramKey}/`, {
  ...
  return (
    <View style={styles.container}>
      ...
      {data?(<><Text style={{ fontSize: 16, right: 100, bottom: 80, marginBottom: 15 }}>
        Product Name:{" "}{data.data.name}
      </Text>
      <Text style={{ fontSize: 16, right: 112, bottom: 80, marginBottom: 15 }}>
        Description:{" "}{data.data.description}
      </Text>
      <Text style={{ fontSize: 16, right: 92, bottom: 80, marginBottom: 15 }}>
        Product Category:{" "}{data.data.category==="908c63a4-9df3-4269-909d-ace6992ea9ee"?"Heavy":"Fragile"}
      </Text>
      <Text style={{ fontSize: 16, right: 125, bottom: 80, marginBottom: 15 }}>
        Quantity:{" "}{data.data.quantity}
      </Text>
      <Text style={{ fontSize: 16, right: 97, bottom: 80, marginBottom: 15 }}>
        Pickup Location:{" "}{data.data.pickup_address}
      </Text>
      <Text style={{ fontSize: 16, right: 105, bottom: 80, marginBottom: 15 }}>
        Drop Location:{" "}{data.data.delivery_address}
      </Text>
      <Text style={{ fontSize: 16, right: 110, bottom: 80, marginBottom: 15 }}>
        Owner Name:{" "}{data.data.pickup_contact_name}
      </Text>
      <Text style={{ fontSize: 16, right: 105, bottom: 80, marginBottom: 15 }}>
        Contact Owner:{" "}{data.data.pickup_contact_phone}
      </Text>
      <Text style={{ fontSize: 16, right: 100, bottom: 80, marginBottom: 15 }}>
        Customer Name:{" "}{data.data.delivery_contact_name}
      </Text>
      <Text style={{ fontSize: 16, right: 94, bottom: 80, marginBottom: 15 }}>
        Contact Customer:{" "}{data.data.delivery_contact_phone}
      </Text>
      <Text style={{ fontSize: 16, right: 115, bottom: 80, marginBottom: 15 }}>
        Delivery Fee:{" "}{data.data.delivery_boy_fee}
      </Text>
      <Text style={{ fontSize: 16, right: 60, bottom: 80, marginBottom: 15 }}>
        Estimated Completion Time:{" "}{getFormattedDuration(data.data.estimated_duration)}
      </Text>
      <Image
        ...
        onPress={() => openGoogleMaps(data.data.pickup_location.coordinates[1], data.data.pickup_location.coordinates[0])}
      >
        <Text
          style={{
            textAlign: "center",
